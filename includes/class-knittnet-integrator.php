...
public function knittnet_generate_image($message, $user_id, $session_id) {
    ...
    $openrouter_api_key = sanitize_text_field($this->options['openrouter_api_key'] ?? '');
    $image_model = sanitize_text_field($this->options['openrouter_image_model'] ?? '');
    $provider = new KnittNet_OpenRouter_Provider($openrouter_api_key);
    $image_response = $provider->generate_image($prompt, array('model' => $image_model));

    $image_url = '';
    if (!empty($image_response['image_base64'])) {
        $image_url = $this->knittnet_save_generated_image($image_response['image_base64'], $image_response['mime_type'] ?? 'image/png');
        if (is_wp_error($image_url)) {
            $image_url = '';
        }
    } elseif (!empty($image_response['image_url'])) {
        $image_url = esc_url_raw($image_response['image_url']);
    }
...
public function knittnet_interpret_search_query($user_query) {
    ...
    $api_key = isset($options['openrouter_api_key']) ? sanitize_text_field($options['openrouter_api_key']) : '';
    $model = isset($options['openrouter_selected_model']) ? sanitize_text_field($options['openrouter_selected_model']) : '';
    ...
    $result = $provider->chat(
        array(
            ...
            array('role' => 'user', 'content' => sanitize_text_field($user_query)),
        ),
        array('model' => $model, 'temperature' => 0.2, 'max_tokens' => 20, 'timeout' => 15)
    );

    return isset($result['content']) ? sanitize_text_field($result['content']) : sanitize_text_field($user_query);
}

/**
 * Convert the colon-style header list returned by knittnet_resolve_custom_provider
 * into the assoc-array form wp_remote_post expects. Used only by the
 * Custom-provider embeddings path (RAG vector search); OpenRouter has no
 * embeddings API of its own.
 */
...
/**
 * Resolve the configured Custom (OpenAI-compatible) embedding endpoint.
 * OpenRouter has no embeddings API, so sites that want a self-hosted or
 * Azure-OpenAI embedding model for RAG vector search still configure it
 * here — this is independent of the OpenRouter-only chat provider.
 */
...
/**
 * Resolve provider transport details for the function-calling loop. Returns
 * null when FC can't run (missing key/model) so the caller falls back to the
 * normal path. OpenRouter is the only provider; it speaks the OpenAI-
 * compatible 'openai' family, which is what knittnet_fc_loop_openai expects.
 */
...
private function knittnet_generate_response($relevant_content, $openrouter_api_key, $conversation_history, $streaming = false, $session_id = '', $testing_data = null, $selected_model = '') {
    try {
        if (!$relevant_content) {
            $error_response = [
        ...
        // OpenRouter's ":online" model suffix grounds the response in live web
        // search results (their hosted web plugin) — no separate search API or
        // key needed, so the "Enable Web Search" toggle still works OpenRouter-only.
        $web_search_enabled = isset($this->options['enable_web_search']) && $this->options['enable_web_search'] === 'on';
        if ($web_search_enabled && $openrouter_selected_model !== '' && substr($openrouter_selected_model, -7) !== ':online') {
            $openrouter_selected_model .= ':online';
        }

        if (empty($openrouter_selected_model)) {
            $error_response = [
                'error' => esc_html__('No OpenRouter model selected. Please select a model in KnittNet → AI Configuration.', 'knittnet'),
